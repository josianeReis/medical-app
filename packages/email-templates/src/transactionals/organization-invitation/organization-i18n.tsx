...
> = ({ invitedByUsername, invitedByEmail, teamName, inviteLink }) => {
  ...
  return (
    <Html>
      <Head />
      <Preview>{t("title", { teamName })}</Preview>
      <Body style={main}>
        <Container style={container}>
          ...
          <Text style={text}>{t("accept_invitation")}</Text>
          <div style={buttonContainer}>
            <Button href={inviteLink} style={button}>
              {t("button_text")}
            </Button>
          </div>
          <Text style={text}>{t("copy_paste_link")}</Text>
          <div style={linkStyle}>
            <Link href={inviteLink} style={linkText}>
              {inviteLink}
            </Link>
          </div>
          <Text style={text}>{t("ignore_email")}</Text>
          <Text style={footer}>{t("automated_message")}</Text>
        </Container>
      </Body>
    </Html>
  );
