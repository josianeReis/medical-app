...
if [ "$SETUP_PERFORMED_ACTIONS" = true ]; then
  ...
  echo ""
  echo "${YELLOW}1. Login to your Infisical environment variables management system (if not done before):${RESET}"
  echo "   Run this command on terminal: ${CYAN}infisical login${RESET}"
  echo ""
